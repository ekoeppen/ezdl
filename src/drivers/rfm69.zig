...
pub fn Rfm69(
    ...
    return struct {
        ...
        const init_values = .{
            ...
            .{ .RssiThresh, RssiThresh{ .rssiThreshold = 100 * 2 } },
            ...
            .{ .SyncValue1, SyncValue1{ .sync_value = 0xf0 } },
            .{ .SyncValue2, SyncValue2{ .sync_value = 0x12 } },
            .{ .SyncValue3, SyncValue3{ .sync_value = 0x78 } },
            .{ .Datamodul, DataModul{
                .data_mode = .packet_mode,
                .modulation_type = .fsk,
                .modulation_shaping = .no_shaping,
            } },
            .{ .PreambleLsb, PreambleLsb{ .preamble_size_lsb = 6 } },
            .{ .FdevMsb, FdevMsb{ .fdev_msb = 3 } },
            .{ .FdevLsb, FdevLsb{ .fdev_lsb = 5 } },
        };

        pub fn init() bool {
            if (!setMode(.stdby)) return false;
            inline for (init_values) |init_value| {
                writeRegister(init_value[0], init_value[1]);
            }
            return true;
        }
        ...
        pub fn packetSent() bool {
            const irqFlags = readRegister(IrqFlags2, .IrqFlags2);
            return irqFlags.packet_sent == 1;
        }

        pub fn packetAvailable() bool {
            const irqFlags = readRegister(IrqFlags2, .IrqFlags2);
